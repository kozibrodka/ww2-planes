package net.kozibrodka.planes.gui;
import net.kozibrodka.planes.entity.EntityPlane;
import net.kozibrodka.planes.entity.EntityPlaneNew;
import net.minecraft.client.gui.screen.ingame.HandledScreen;
import net.minecraft.entity.player.PlayerInventory;
import org.lwjgl.opengl.GL11;

public class GuiPlaneNew extends HandledScreen
{

    public GuiPlaneNew(PlayerInventory inventoryplayer, EntityPlaneNew entityplane)
    {
        super(new InventoryPlaneNew(inventoryplayer, entityplane));
        plane = entityplane;
    }

    protected void drawForeground()
    {
        textRenderer.draw(plane.plane.name, 60, 6, 0x404040);
        textRenderer.draw("Inventory", 8, (backgroundHeight - 96) + 2, 0x404040);
        if(plane.plane.numCargoSlots > 0)
        {
            textRenderer.draw("Cargo:", 36, 22, 0x404040);
        }
        if(plane.plane.numBulletSlots > 0)
        {
            textRenderer.draw("Bullets:", 36, 40, 0x404040);
        }
        if(plane.plane.numBombSlots > 0)
        {
            textRenderer.draw("Bombs:", 36, 58, 0x404040);
        }
        //addon
        if(plane.plane.numRocketSlots > 0)
        {
            textRenderer.draw("Rockets:", 36, 22, 0x404040);
        }
        //addon
    }

    protected void drawBackground(float f)
    {
        int i = minecraft.textureManager.getTextureId("/assets/planes/stationapi/textures/gui/plane.png");
        GL11.glColor4f(1.0F, 1.0F, 1.0F, 1.0F);
        minecraft.textureManager.bindTexture(i);
        int j = (width - backgroundWidth) / 2;
        int k = (height - backgroundHeight) / 2;
        drawTexture(j, k, 0, 0, backgroundWidth, backgroundHeight);
        if(plane.isFuelled())
        {
            int l = plane.getBurnTimeRemainingScaled(12);
            drawTexture(j + 8, (k + 36 + 12) - l, 176, 12 - l, 14, l + 2);
        }
        if(plane.plane.numRocketSlots > 0)
        {
            for(int i1 = plane.plane.numRocketSlots; i1 < 5; i1++)
            {
                drawTexture(j + 79 + 18 * i1, k + 17, 190, 0, 18, 18);
            }
        }else {
            for (int i1 = plane.plane.numCargoSlots; i1 < 5; i1++) {
                drawTexture(j + 79 + 18 * i1, k + 17, 190, 0, 18, 18);
            }
        }

        for(int j1 = plane.plane.numBulletSlots; j1 < 5; j1++)
        {
            drawTexture(j + 79 + 18 * j1, k + 35, 190, 0, 18, 18);
        }

        for(int k1 = plane.plane.numBombSlots; k1 < 5; k1++)
        {
            drawTexture(j + 79 + 18 * k1, k + 53, 190, 0, 18, 18);
        }
        //addon
//        for(int i1 = plane.plane.numRocketSlots; i1 < 5; i1++)
//        {
//            blit(j + 79 + 18 * i1, k + 17, 190, 0, 18, 18);
//        }
        //addon
    }

    private EntityPlaneNew plane;
}
