...
public class EntityPlaneNew extends Entity
        implements Inventory, WW2Plane
{

    public EntityPlaneNew(World world)
    {
        super(world);
        boatCurrentDamage = 0;
        boatTimeSinceHit = 0;
...
//        axes = new RotatedAxes();
    ...
    public EntityPlaneNew(World world, double d, double d1, double d2)
    {
        this(world);
        standingEyeHeight = height / 2.0F - 0.17F;
    ...
    public EntityPlaneNew(World world, double d, double d1, double d2,
                          PlayerEntity entityplayer, int i, PlaneTypeNew planetype)
    {
        this(world);
        plane = planetype;
        standingEyeHeight = plane.standingOko;
        setBoundingBoxSpacing(plane.planeWidth, plane.planeHeight);
        setPosition(d, d1 + (double)standingEyeHeight, d2);
        velocityX = 0.0D;
        velocityY = 0.0D;
        velocityZ = 0.0D;
...
//                ModLoader.OpenGUI((EntityPlayer)passenger, new GuiPlaneController(this));
        ...
        for(int k = 0; k < j; k++)
        {
            ...
            {
                world.addParticle("largesmoke", x + d7, y + d8, z + d9, velocityX, velocityY, velocityZ);
            } else
            {
        ...
        if(list != null && list.size() > 0)
        {
            for(int j1 = 0; j1 < list.size(); j1++)
            {
                ...
                if(flag && entity != passenger && entity.isPushable() && (entity instanceof EntityPlaneNew))
                {
    ...
    public void inventoryKey(Minecraft minecraft, PlayerEntity entityplayer) {
        if(!world.isRemote) {
            if (SdkTools.minecraft.currentScreen instanceof GuiPlane) {
                ...
                SdkTools.minecraft.setScreen(new GuiPlaneNew(((PlayerEntity)passenger).inventory, (EntityPlaneNew)passenger.vehicle));
            }
        }
    }
