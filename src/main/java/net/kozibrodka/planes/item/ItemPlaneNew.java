...
public class ItemPlaneNew extends TemplateItemBase
{

    public ItemPlaneNew(Identifier i, String s)
    {
        super(i);
        maxStackSize = 1;
        planeType = s;
    ...
    public ItemInstance use(ItemInstance itemstack, Level world, PlayerBase entityplayer)
    {
        ...
        if(movingobjectposition.type == HitType.field_789)
        {
            ...
            PlaneTypeNew planetype = mod_Planes.getPlaneTypeNew(planeType);
            if(!world.isServerSide)
            {
                int l = itemstack.getDamage();
                world.spawnEntity(new EntityPlaneNew(world, (double)i + 0.5D, (double)j + 1.5D, (double)k + 0.5D, entityplayer, l, planetype));
            }
            itemstack.count--;
        }
        return itemstack;
